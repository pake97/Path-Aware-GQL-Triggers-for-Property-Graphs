...
public class Experiment4Test {
    ...
    @Test
    void runExperiment4() throws IOException {
        System.out.println("Experiment 4: Monitoring Overhead vs N — 1 firing trigger + N-1 non-firing triggers");
        System.out.println("Real pattern    : " + PATTERN);
        System.out.println("Impossible pattern: " + IMPOSSIBLE_PATTERN);
        System.out.println("Mode format: <type>-<total_triggers>  (only 1 trigger fires per run)");
        System.out.println();
        System.out.println(
        ...
        for (int nNonFiring : NON_FIRING_COUNTS) {
            int total = 1 + nNonFiring;
            System.err.println("  INDEX total=" + total + " (1 firing + " + nNonFiring + " non-firing)");
            runWithTrigger("INDEX-" + total, nNonFiring);
        }

        // 4. PATH (Automaton): same structure
        System.err.println("--- Starting Path Phase ---");
        switchRegistry(TriggerType.AUTOMATON);
        for (int nNonFiring : NON_FIRING_COUNTS) {
            int total = 1 + nNonFiring;
            System.err.println("  PATH total=" + total + " (1 firing + " + nNonFiring + " non-firing)");
            runWithTrigger("PATH-" + total, nNonFiring);
        }
    }

    private void runWithTrigger(String modeLabel, int nNonFiring) throws IOException {
        clearDatabase();

        TriggerRegistryInterface.PathActivation realActivation = new TriggerRegistryInterface.PathActivation(
                PATTERN, 0, TriggerRegistryInterface.EventType.ON_CREATE);
        TriggerRegistryInterface.PathActivation impossibleActivation = new TriggerRegistryInterface.PathActivation(
                IMPOSSIBLE_PATTERN, 0, TriggerRegistryInterface.EventType.ON_CREATE);
        ...
        // 1 trigger on the real pattern — this is the one that fires
        triggerIds.add(orchestrator.register(new FullTrigger(
                modeLabel.toLowerCase() + "-real-trigger",
        ...
        // nNonFiring triggers on the impossible pattern — these never fire
        for (int i = 1; i <= nNonFiring; i++) {
            triggerIds.add(orchestrator.register(new FullTrigger(
                    modeLabel.toLowerCase() + "-nonfiring-trigger-" + i,
                    TriggerRegistryInterface.Scope.PATH,
                    impossibleActivation,
                    100,
                    true,
                    ctx -> true,
                    committed -> {
