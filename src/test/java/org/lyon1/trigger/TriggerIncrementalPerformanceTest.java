...
public class TriggerIncrementalPerformanceTest {
    ...
    @Test
    void runTriggerPerformanceTest() throws IOException {
        System.out.println("Performance Comparison (With vs Without Trigger)");
        System.out.println("Trigger: (:Person)-[:own]->(:Account)<-[:deposit]-(:Loan)");
        System.out.println();
        System.out.println(
        ...
        // 1. Warmup (No measurement)
        System.out.println("--- Starting Warmup Phase ---");
        clearDatabase();
        runFullSequence(false, null, null, true); // Warmup WITHOUT trigger
        clearDatabase();
        runFullSequence(true, null, null, true); // Warmup WITH trigger (no count needed)

        // 2. Measure WITHOUT Trigger
        System.out.println("--- Starting Run WITHOUT Trigger (Baseline) ---");
        clearDatabase();
        runFullSequence(false, null, null, false);

        // 3. Measure WITH Trigger
        System.out.println("--- Starting Run WITH Trigger ---");
        clearDatabase();
        ...
        runFullSequence(true, lastTriggerDetectedTime, triggerCount, false);
    ...
    private void runFullSequence(boolean withTrigger, AtomicLong lastTriggerDetectedTime, AtomicLong triggerCount,
        ...
        measureQuery("tw-1.cypher", "AddPersonWrite1.csv",
                ...
                new String[] { "personId", "personName", "isBlocked", "gender", "birthday", "country", "city",
                        "createTime" },
                withTrigger, lastTriggerDetectedTime, triggerCount, isWarmup);
    ...
    // Static map to store baseline average times for overhead calculation
    ...
    private void measureQuery(String queryFile, String csvFile, String[] paramKeys, String[] csvHeaders,
            boolean withTrigger, AtomicLong lastTriggerDetectedTime, AtomicLong triggerCount, boolean isWarmup)
            throws IOException {
        ...
        for (Map<String, Object> params : dataset) {
            ...
            if (withTrigger && lastTriggerDetectedTime != null && lastTriggerDetectedTime.get() > startTime) {
                // Activation latency typically measured from commit to trigger firing
                long latency = lastTriggerDetectedTime.get() - endTime;
        ...
        System.out.printf("%-13s | %-6s | %5d | %16.4f | %11.4f | %20.4f | %14.4f | %11d | %12.2f | %12.2f | %10.4f\n",
                queryFile, withTrigger ? "WITH" : "WITHOUT", count, avgTxMs, stdDevTx, avgActMs, stdDevAct,
                totalActivations, Math.max(0.0, overhead), finalAvgMem, stdDevMem);
    }
}
