...
public class Experiment5Test {
    ...
    // Pool of N_MAX distinct, non-overlapping patterns.
    // PATTERNS[0] is the only one that fires (completed by tw-15).
    // PATTERNS[1..9] are all impossible given our query sequence
    //   (relationships used: Person-[:own]->Account, Person-[:apply]->Loan, Loan-[:deposit]->Account).
    // For a run with N triggers, the first N patterns from this pool are used.
    // This means exactly 1 trigger fires regardless of N, mirroring Experiment4 for direct comparison.
    private static final String[] PATTERNS = {
        "(:Person)-[:own]->(:Account)<-[:deposit]-(:Loan)",  // [0] FIRES — completed by tw-15
        "(:Person)-[:own]->(:Person)",                        // [1] silent — own goes Person->Account
        "(:Account)-[:own]->(:Account)",                      // [2] silent — wrong source node
        "(:Loan)-[:deposit]->(:Person)",                      // [3] silent — deposit goes Loan->Account
        "(:Account)-[:deposit]->(:Loan)",                     // [4] silent — reversed direction
        "(:Loan)-[:own]->(:Account)",                         // [5] silent — own goes Person->Account
        "(:Person)-[:deposit]->(:Account)",                   // [6] silent — deposit goes Loan->Account
        "(:Account)-[:own]->(:Loan)",                         // [7] silent — impossible node combo
        "(:Loan)-[:own]->(:Person)",                          // [8] silent — impossible node combo
        "(:Person)-[:deposit]->(:Loan)",                      // [9] silent — impossible node combo
    };

    // N values mirror Experiment3 and Experiment4 for direct comparison
    private static final int[] N_VALUES = {1, 3, 5, 10};
    ...
    @Test
    void runExperiment5() throws IOException {
        System.out.println("Experiment 5: Discrimination Cost — N triggers on distinct non-overlapping patterns");
        System.out.println("Only PATTERNS[0] fires; PATTERNS[1..N-1] are distinct and silent.");
        System.out.println("Compare with Experiment4 (same N, identical silent patterns) to isolate discrimination cost.");
        System.out.println("Mode format: <type>-<N>  (1 trigger fires, N-1 are silent with distinct patterns)");
        System.out.println();
        for (int i = 0; i < PATTERNS.length; i++) {
            System.out.println("  [" + i + "] " + PATTERNS[i] + (i == 0 ? "  <- fires" : "  <- silent"));
        }
        System.out.println();
        System.out.println(
        ...
        // 3. INDEX with N = 1, 3, 5, 10 distinct patterns
        System.err.println("--- Starting Index Phase ---");
        switchRegistry(TriggerType.INDEX);
        for (int n : N_VALUES) {
            System.err.println("  INDEX N=" + n + " (1 firing + " + (n - 1) + " silent distinct)");
            runWithTrigger("INDEX-" + n, n);
        }

        // 4. PATH (Automaton) with N = 1, 3, 5, 10 distinct patterns
        System.err.println("--- Starting Path Phase ---");
        switchRegistry(TriggerType.AUTOMATON);
        for (int n : N_VALUES) {
            System.err.println("  PATH N=" + n + " (1 firing + " + (n - 1) + " silent distinct)");
    ...
    private void runWithTrigger(String modeLabel, int n) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            TriggerRegistryInterface.PathActivation activation = new TriggerRegistryInterface.PathActivation(
                    PATTERNS[i], 0, TriggerRegistryInterface.EventType.ON_CREATE);
            triggerIds.add(orchestrator.register(new FullTrigger(
    ...
    private void measureQuery(String queryFile, String csvFile, String[] paramKeys, String[] csvHeaders,
        ...
        for (Map<String, Object> params : dataset) {
            ...
            if (withTrigger && lastTriggerDetectedTime != null && lastTriggerDetectedTime.get() > commitStartTime) {
                // Only PATTERNS[0] fires, so this latency is purely from the real trigger
