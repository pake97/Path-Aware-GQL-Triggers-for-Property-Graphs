...
public class IndexTriggerIncrementalPerformanceTest {
    ...
    @Test
    void runIndexPerformanceTest() throws IOException {
        System.out.println("Performance Comparison (INDEX - With vs Without Trigger)");
    ...
    private void measureQuery(String queryFile, String csvFile, String[] paramKeys, String[] csvHeaders,
        ...
        double stdDevMem = Math.sqrt(memorySamples.stream()
                .mapToDouble(m -> Math.pow(m - avgMem, 2)).average().orElse(0.0));
        ...
        System.out.printf("%-13s | %-8s | %5d | %16.4f | %11.4f | %20.4f | %14.4f | %11d | %12.2f | %12.2f | %10.4f\n",
                queryFile, withTrigger ? "INDEX" : "BASELINE", count, avgTxMs, stdDevTx,
                avgActMs, stdDevAct, totalActivations, Math.max(0.0, overhead), Math.max(0, avgMem), stdDevMem);
    }
}
